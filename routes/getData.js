module.exports = function(pool, maxLabel) {
  ...
  router.get("/getCustomData/:category/:previousValue?", (req, res) => {
    ...
    if (req.params.category === "project") {
      ...
      mainData = "select build_id, buildno from buildno where pj_id = " + req.params.previousValue + ";";
    } else if (req.params.category === "class") {
      mainData = "select class_id, package_name, class_name from class where build_id = " + req.params.previousValue + ";";
    } else if (req.params.category === "testcase") {
      mainData = "select method_id, method_name from method where class_id = " + req.params.previousValue + ";";
    } else {
      mainData = "";
    }
  ...
  router.get("/getInitialModalData/:pj_id", (req, res) => {
    ...
    let labelData = "select pj_name, pj_team, pj_platform, pj_author, pj_id, pj_link from project where pj_id = '" + req.params.pj_id + "';";
    let result = {};
